...
export async function domainFactory({ name, revision = 1 }, { system = "rfx1", busUrl, logger, snapshotThreshold = 50 } = {}) {
  ...
  const spi = {
    ...
    async trackInstance(instance) {
      ...
      _domain.bus.channel("instance:tracked").publish("track", { id: instance.id, data: indexed })
    },
    async removeInstance(instance) {
      ...
      if (removed) {
        _domain.bus.channel("instance:tracked").publish("remove", { id: instance.id, ...removed })
      }
      return removed
    },
    ...
    publish(eventKey, payload, channel = "events") {
      return _domain.bus.channel([_domain.id, channel]).publish(eventKey, payload)
    },
  ...
  const api = {
    ...
    addEffect(effect, activator, generator) {
      _domain.effects.push({ effect, activator, generator })
    },
    /**
     *
     * @param {*} id
