 ...
 */
export async function domainFactory({ name, revision = 1 }, { system = "rfx1", busUrl, logger, host, snapshotThreshold = 50 } = {}) {
  const _domain = {
  ...
  const api = {
    ...
    addHook(hook, activator) {
      _domain.hooks.push({ hook, activator })
    },
    /**
     *
     * @param {*} id
