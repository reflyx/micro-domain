...
export async function storeFactory(storeId, { height = 0, locale, snapshotThreshold, readOnly = false }, { url, bus, instance }) {
          ...
          mutate(mutation) {
            return set(state => {
              ...
              if (patches.length > 0) {
                ...
                writeConn.xadd(`${storeId}:frames`, "*", "frame", JSON.stringify(frame))
                _state.changes.next({ oldState: state, newState: nextState, frame })
                _state.height = frame.height
                return nextState
              }
            })
  ...
  if (!readOnly) {
    _state.mutationsSubscription = (await bus.stream(`${storeId}:mutations`).consume("mutators")).subscribe(request => {
      if (request[0] === "mutate") {
  ...
  return {
    ...
    changes() {
      return _state.changes.asObservable()
    },
